...
{
    /// <summary>
    /// Designer layout for <see cref="Shaker.SQLLiteDB.Activities.Activities.Maintenance.SQLiteMaintenance"/>: which fields are drawn inside the
    /// activity itself, in what order, and how each one is edited. Generated - see tools/generate-viewmodels.
    /// </summary>
    public class SQLiteMaintenanceViewModel : DesignPropertiesViewModel
    {
        public SQLiteMaintenanceViewModel(IDesignServices services)
            : base(services)
        {
        }
        ...
        public DesignProperty<Shaker.SQLLiteDB.Activities.Core.SQLiteMaintenanceOperation> Operation { get; set; } = new DesignProperty<Shaker.SQLLiteDB.Activities.Core.SQLiteMaintenanceOperation>();
        ...
        public DesignOutArgument<bool> IsHealthy { get; set; } = new DesignOutArgument<bool>();
        ...
        protected override void InitializeModel()
        {
            ...
            Operation.Tooltip = "Which maintenance command to run.";
            Operation.Category = "Input";
            Operation.IsPrincipal = true;
            Operation.OrderIndex = order++;
            Operation.Widget = new DefaultWidget { Type = "Dropdown" };
            Operation.DataSource = EnumDataSourceBuilder<Shaker.SQLLiteDB.Activities.Core.SQLiteMaintenanceOperation>.Build(DataSourceEnumOrder.OrderById, value => value.ToString());

            Result.DisplayName = "Report";
            Result.Tooltip = "What the operation reported.";
            ...
            IsHealthy.DisplayName = "Is healthy";
            IsHealthy.Tooltip = "For the integrity and foreign key checks: true when the database reported no problem.";
            IsHealthy.Category = "Output";
            IsHealthy.IsVisible = false;
            IsHealthy.OrderIndex = order++;
            IsHealthy.Widget = new DefaultWidget { Type = "Checkbox" };
