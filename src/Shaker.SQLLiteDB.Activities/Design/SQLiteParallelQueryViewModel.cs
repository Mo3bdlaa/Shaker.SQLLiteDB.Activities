...
{
    /// <summary>
    /// Designer layout for <see cref="Shaker.SQLLiteDB.Activities.Activities.Query.SQLiteParallelQuery"/>: which fields are drawn inside the
    /// activity itself, in what order, and how each one is edited. Generated - see tools/generate-viewmodels.
    /// </summary>
    public class SQLiteParallelQueryViewModel : DesignPropertiesViewModel
    {
        public SQLiteParallelQueryViewModel(IDesignServices services)
            : base(services)
        {
        }
        ...
        public DesignInArgument<System.Collections.Generic.IDictionary<string, string>> Queries { get; set; } = new DesignInArgument<System.Collections.Generic.IDictionary<string, string>>();

        public DesignOutArgument<System.Collections.Generic.Dictionary<string, System.Data.DataTable>> Result { get; set; } = new DesignOutArgument<System.Collections.Generic.Dictionary<string, System.Data.DataTable>>();
        ...
        public DesignInArgument<int> MaxDegreeOfParallelism { get; set; } = new DesignInArgument<int>();

        public DesignProperty<bool> OpenReadOnly { get; set; } = new DesignProperty<bool>();

        public DesignProperty<Shaker.SQLLiteDB.Activities.Core.SQLiteColumnTyping> ColumnTyping { get; set; } = new DesignProperty<Shaker.SQLLiteDB.Activities.Core.SQLiteColumnTyping>();
        ...
        public DesignOutArgument<int> TotalRows { get; set; } = new DesignOutArgument<int>();
        ...
        protected override void InitializeModel()
        {
            ...
            Queries.Tooltip = "The queries to run, as a Dictionary(Of String, String): the key names the result, the value is the SELECT statement.";
            Queries.Category = "Input";
            Queries.IsPrincipal = true;
            Queries.IsRequired = true;
            Queries.OrderIndex = order++;
            Queries.Widget = new DefaultWidget { Type = "Input" };

            Result.DisplayName = "Results";
            Result.Tooltip = "One DataTable per query, keyed by the name you gave it.";
            ...
            MaxDegreeOfParallelism.DisplayName = "Max parallel readers";
            MaxDegreeOfParallelism.Tooltip = "How many queries run at the same time. Default 4.";
            MaxDegreeOfParallelism.Category = "Options";
            MaxDegreeOfParallelism.IsVisible = false;
            MaxDegreeOfParallelism.OrderIndex = order++;
            MaxDegreeOfParallelism.Widget = new DefaultWidget { Type = "Input" };

            OpenReadOnly.DisplayName = "Open read only";
            OpenReadOnly.Tooltip = "Open the extra connections read only, so they can never take the writer lock.";
            OpenReadOnly.Category = "Options";
            OpenReadOnly.IsVisible = false;
            OpenReadOnly.OrderIndex = order++;
            OpenReadOnly.Widget = new DefaultWidget { Type = "Checkbox" };

            ColumnTyping.DisplayName = "Column typing";
            ColumnTyping.Tooltip = "Auto derives each column type from the values returned, Declared trusts the provider, AllText returns every column as text.";
            ColumnTyping.Category = "Options";
            ColumnTyping.IsVisible = false;
            ColumnTyping.OrderIndex = order++;
            ColumnTyping.Widget = new DefaultWidget { Type = "Dropdown" };
            ColumnTyping.DataSource = EnumDataSourceBuilder<Shaker.SQLLiteDB.Activities.Core.SQLiteColumnTyping>.Build(DataSourceEnumOrder.OrderById, value => value.ToString());
            ...
            TotalRows.DisplayName = "Total rows";
            TotalRows.Tooltip = "Sum of the row counts of all results.";
            TotalRows.Category = "Output";
            TotalRows.IsVisible = false;
            TotalRows.OrderIndex = order++;
            TotalRows.Widget = new DefaultWidget { Type = "Input" };
