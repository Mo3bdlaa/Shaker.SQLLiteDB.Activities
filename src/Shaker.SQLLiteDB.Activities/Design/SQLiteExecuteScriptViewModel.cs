...
{
    /// <summary>
    /// Designer layout for <see cref="Shaker.SQLLiteDB.Activities.Activities.Write.SQLiteExecuteScript"/>: which fields are drawn inside the
    /// activity itself, in what order, and how each one is edited. Generated - see tools/generate-viewmodels.
    /// </summary>
    public class SQLiteExecuteScriptViewModel : DesignPropertiesViewModel
    {
        public SQLiteExecuteScriptViewModel(IDesignServices services)
            : base(services)
        {
        }
        ...
        public DesignInArgument<string> Script { get; set; } = new DesignInArgument<string>();

        public DesignInArgument<string> ScriptFilePath { get; set; } = new DesignInArgument<string>();
        ...
        protected override void InitializeModel()
        {
            ...
            Script.Tooltip = "The SQL script. Statements are separated by semicolons. Ignored when 'Script file path' is set.";
            Script.Category = "Input";
            Script.IsPrincipal = true;
            Script.OrderIndex = order++;
            Script.Widget = new DefaultWidget { Type = "TextComposer" };

            ScriptFilePath.DisplayName = "Script file path";
            ScriptFilePath.Tooltip = "Path of a .sql file to run. Wins over 'Script'.";
            ScriptFilePath.Category = "Input";
            ScriptFilePath.IsPrincipal = true;
            ScriptFilePath.OrderIndex = order++;
            ScriptFilePath.Widget = new DefaultWidget { Type = "Input" };

            Result.DisplayName = "Statements run";
            Result.Tooltip = "How many statements the script executed.";
            ...
            UseTransaction.Tooltip = "Wrap the whole script in one transaction, so a failure halfway leaves the database untouched.";
            UseTransaction.Category = "Options";
            UseTransaction.IsVisible = false;
