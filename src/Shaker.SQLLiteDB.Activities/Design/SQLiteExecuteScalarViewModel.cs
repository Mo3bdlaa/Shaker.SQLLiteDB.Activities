...
{
    /// <summary>
    /// Designer layout for <see cref="Shaker.SQLLiteDB.Activities.Activities.Query.SQLiteExecuteScalar"/>: which fields are drawn inside the
    /// activity itself, in what order, and how each one is edited. Generated - see tools/generate-viewmodels.
    /// </summary>
    public class SQLiteExecuteScalarViewModel : DesignPropertiesViewModel
    {
        public SQLiteExecuteScalarViewModel(IDesignServices services)
            : base(services)
        {
        }
        ...
        public DesignOutArgument<object> Result { get; set; } = new DesignOutArgument<object>();
        ...
        public DesignOutArgument<string> TextResult { get; set; } = new DesignOutArgument<string>();

        public DesignOutArgument<double> NumberResult { get; set; } = new DesignOutArgument<double>();

        public DesignOutArgument<bool> IsNull { get; set; } = new DesignOutArgument<bool>();
        ...
        protected override void InitializeModel()
        {
            ...
            Result.Tooltip = "The first value of the first row.";
            ...
            TextResult.DisplayName = "Text result";
            TextResult.Tooltip = "The value as text, which saves a conversion in the workflow.";
            TextResult.Category = "Output";
            TextResult.IsVisible = false;
            TextResult.OrderIndex = order++;
            TextResult.Widget = new DefaultWidget { Type = "Input" };

            NumberResult.DisplayName = "Number result";
            NumberResult.Tooltip = "The value as a number. 0 when the value is NULL or not numeric.";
            NumberResult.Category = "Output";
            NumberResult.IsVisible = false;
            NumberResult.OrderIndex = order++;
            NumberResult.Widget = new DefaultWidget { Type = "Input" };

            IsNull.DisplayName = "Is null";
            IsNull.Tooltip = "True when the query returned NULL or no row at all.";
            IsNull.Category = "Output";
            IsNull.IsVisible = false;
            IsNull.OrderIndex = order++;
            IsNull.Widget = new DefaultWidget { Type = "Checkbox" };
